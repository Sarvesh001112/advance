    ...
    while True:
        ...
        if right == "" and left == "":
            if x > 500:
                ...
                if x < 200:
                    ...
                    x = 300
                    right, left = "", ""
                    cv2.imwrite(f"visitors/in/{datetime.now().strftime('%H-%M-%S')}.jpg", frame1)
                    # now=datetime.now()
        ...
        # string=datetime.datetime.strptime('2-1-2020 3:14:5', '%m-%d-%Y %H:%M:%S')
                ...
                if x > 500:
                    ...
                    x = 300
                    right, left = "", ""
                    cv2.imwrite(f"visitors/out/{datetime.now().strftime('%H-%M-%S')}.jpg", frame1)
